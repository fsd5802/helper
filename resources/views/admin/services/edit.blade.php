...
@section('module_name', trans('admin.header.services'))
@section('index_route', getRoute('services.index'))
@section('store_route', getRoute('services.update', $service))
@section('page_type', trans('general.edit'))
@section('form_type', 'POST')
@section('title') @lang('admin.header.services') @endsection
...
@section('fields_content')
                ...
                @foreach (config('translatable.locales') as $key => $locale)
                    <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}" role="tabpanel">
                        <div class="col form-group">
                            ...
                            @enderror" value="{{ old($locale . '.name', $service->translate($locale)->name) }}">
                            @error("$locale.name")
                    ...
                    {{ old($locale . '.desc', strip_tags($service->translate($locale)->desc)) }}
                </textarea>
                <script>
    ...
    <div class="col form-group">
        @if (isset($service->image))
            ...
            <br>
            <img src="{{ asset($service->image) }}" class="w-25">
        @endif
    </div>
    <div class="col form-group">
        <label>@lang('slider.image') <span class="text-danger">*</span></label>
