...
@section('store_route', getRoute('blogs.update', $blog))
    ...
    <div class="card-body">
            ...
            <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}" role="tabpanel">
                <div class="col form-group">
                    ...
                    <input type="text" name="{{ $locale . '[name]' }}" id="{{ $locale . '[name]' }}"
                        ...
                        @enderror" value="{{ old($locale . '.name', $blog->translate($locale)->name) }}">
                    @error("$locale.name")
                ...
                <div class="col form-group">
                    ...
                    {{ old($locale . '.desc', strip_tags($blog->translate($locale)->desc)) }}
                </textarea>
                    <script>
        ...
        <div class="col form-group">
            ...
            <select id="work_id" class="form-control" name="work_id">
                ...
                <option value="{{ $work->id }}" {{ old('work_id', $blog->work_id )== $work->id ? 'selected':'' }}>{{
                    $work->name }}</option>
                @endforeach
                @endif
            </select>
